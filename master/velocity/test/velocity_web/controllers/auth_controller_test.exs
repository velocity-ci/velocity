defmodule Velocity.AuthControllerTest do
  use VelocityWeb.ConnCase, async: true

  setup %{conn: conn} do
    {:ok, [conn: put_req_header(conn, "accept", "application/json")]}
  end
  test "creates and renders auth token when user data is valid",
       %{conn: conn} do
    conn
    |> recycle()
  ...
  test "does not render auth token when username does not exist",
       %{conn: conn} do
    conn
    |> recycle()
    |> post("/api/v1/auth", %{username: "bob", password: "password1234"})
