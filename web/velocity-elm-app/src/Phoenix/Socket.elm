module Phoenix.Socket exposing
    ( Socket, Msg, init, withDebug, withoutHeartbeat, withHeartbeatInterval, update, listen, map
    , join, leave
    , on, off
    , push
    )
...
import Phoenix.Helpers exposing (Message, emptyPayload, encodeMessage, messageDecoder)
...
send { ref } event channel payload =
    sendMessage (Message event channel payload (Just ref))


sendMessage : Message -> Cmd (Msg msg)
sendMessage message =
    WebSocket.send (encodeMessage message)
...
phoenixMessages socket =
    WebSocket.listen decodeMessage
                                    ...
                                    Dict.get ref socket.pushes
                                        |> andThen
                                            (\push_ ->
                                                case status of
                                                    "ok" ->
                                                        Maybe.map (\f -> (ExternalMsg << f) response) push_.onOk

                                                    "error" ->
                                                        Maybe.map (\f -> (ExternalMsg << f) response) push_.onError
...
map fn socket =
    ...
    , pushes = Dict.map (\_ push_ -> Push.map fn push_) socket.pushes
    , ref = socket.ref
    , heartbeatIntervalSeconds = socket.heartbeatIntervalSeconds
